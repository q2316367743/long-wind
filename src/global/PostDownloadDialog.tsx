...
export async function openPostDownloadDialog(url = ''): Promise<void> {
  ...
  const data = ref<PostDownload>({
    ...
    referer: '',
    cookie: '',
    proxy: settingsStore.state.useProxy ? settingsStore.state.proxy || '' : ''
  })
...
· 支持HTTP/HTTPS、M3U8、MPD链接"
        ...
        <Form layout={'inline'} class={'mb-8px'}>
          <FormItem label={'重命名'}>
            <Input
              ...
              placeholder="选填，M3U8文件默认使用时间戳命名，多个文件时自动加上序号"
            ></Input>
          </FormItem>
          <FormItem label={'分片数'}>
        ...
        {advance.value && (
          <>
            <FormItem label={'User-Agent'}>
              <Textarea
                v-model={data.value.userAgent}
                autosize={{ minRows: 2, maxRows: 3 }}
              ></Textarea>
            </FormItem>
            <FormItem label={'Authorization'}>
              <Textarea
                v-model={data.value.authorization}
                autosize={{ minRows: 2, maxRows: 3 }}
              ></Textarea>
            </FormItem>
            <FormItem label={'Referer'}>
              <Textarea
                v-model={data.value.referer}
                autosize={{ minRows: 2, maxRows: 3 }}
              ></Textarea>
            </FormItem>
            <FormItem label={'Cookie'}>
              <Textarea
                v-model={data.value.cookie}
                autosize={{ minRows: 2, maxRows: 3 }}
              ></Textarea>
            </FormItem>
            <FormItem label={'代理'}>
    ...
    footer: () => (
      <div class={'flex justify-between items-center'}>
        ...
        <Space size={'small'}>
          <Button theme={'default'} disabled={submitting.value} onClick={handleCancel}>
            取消
          </Button>
          <Button theme={'primary'} loading={submitting.value} onClick={handleSubmit}>
            提交
          </Button>
        </Space>
      </div>
